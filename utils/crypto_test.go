...
func BenchmarkEncryptString(b *testing.B) {
	for n := 0; n < b.N; n++ {
		_, _ = EncryptString("The secular cooling that must someday overtake our planet has already gone far indeed with our neighbour.", "testcharacters")
	}
}

func BenchmarkDecryptString(b *testing.B) {
	for n := 0; n < b.N; n++ {
		_, _ = DecryptString("The secular cooling that must someday overtake our planet has already gone far indeed with our neighbour.", "testcharacters")
	}
}
